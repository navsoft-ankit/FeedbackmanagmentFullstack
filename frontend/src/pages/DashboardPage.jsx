  ...
  const fetchStats = async () => {
    try {
      const [formsRes, usersRes] = await Promise.all([
        api.get(`/forms/user-stats?email=${email}`),
        api.get(`/auth/active-users-count`),
      ]);

      setStats({
        totalForms: formsRes.data.availableForms,
        totalFeedbacks: formsRes.data.submittedForms,
      });
          ...
          <>
            <div className="hero-card">
              <div>
                <h2>Welcome User</h2>
                <p>
                  Fill feedback forms and submit responses quickly using the smart system.
                </p>
